...
BOT_TOKEN = "MTMzNzMyOTE1NjQxNjg2ODQxNA.GFZq4-.Wcli3EEtsDXcrRzAzXmZGAWFV4yoWCC4sTMtnU"  # Replace with your actual bot token
...
class TicketCloseView(View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.followup.send("Ticket has been closed. Deleting in 5 seconds...", ephemeral=True)
...
class ReopenTicketView(View):
    ...
    @discord.ui.button(label="Reopen Ticket", style=discord.ButtonStyle.green, custom_id="reopen_ticket")
    async def reopen_ticket(self, interaction: discord.Interaction, button: Button):
        ...
        ticket_category = get_ticket_category(guild)
        if not ticket_category:
            await interaction.response.send_message("Ticket category not found. Please create it.", ephemeral=True)
            return

        await channel.edit(category=ticket_category, sync_permissions=True)
        await interaction.response.send_message("Ticket has been reopened.", ephemeral=True)
